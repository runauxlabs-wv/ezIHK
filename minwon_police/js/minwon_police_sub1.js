...
//창띄우기
var app = document.querySelectorAll("a.app");
var basket = document.querySelectorAll("a.basket");
// var minwonName = document.querySelectorAll();

[].forEach.call(app, function (apps) {
    apps.addEventListener('click', function (e) {
        alert('민원신청시에는 본인인증이 필요합니다.');
        location.href = "/portfolio/minwon_police/html/minwon_police_login.html";
        e.preventDefault();
    });
});
[].forEach.call(basket, function (baskets) {
    baskets.addEventListener('click', function (e) {
        confirm('선택하신 민원을 민원바구니에 담으시겠습니까?');
        // confirm(`선택하신 민원 '${minwonName}'을(를) 민원바구니에 담으시겠습니까?`);
        e.preventDefault();
    });
});
...
if (window.innerWidth < 1200) {
    ...
    // [].forEach.call(subMenu, function (subMenus) {
...
$(function () {
    ...
    // tab
    $("dd.context").hide().eq(0).show(); // 모든dd숨기고 첫번째만 열기
    $(".subTab li").click(function () {
        var conBox = "#" + $(this).data('id'); // 탭에서 짝찾아 변수처리
        $(this).addClass('on').siblings().removeClass('on');
        $(conBox).addClass('on').siblings().removeClass('on');
        $("dd.context", conBox).eq(0).show(); // 해당 짝의 콘텐츠에서 dd 첫번째 열기
    });
